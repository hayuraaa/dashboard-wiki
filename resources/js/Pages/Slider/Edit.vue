...
const form = useForm({
    slider_category_id: props.slider.slider_category_id,
    nama: props.slider.nama,
    keterangan: props.slider.keterangan,
    gambar: null,
    urutan: props.slider.urutan,
    is_active: props.slider.is_active,
    _method: 'PUT'
});

const previewImage = ref(null);
const existingImage = ref(props.slider.gambar);
...
<template>
    ...
    <AdminLayout>
        <template #title>Edit Slider</template>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50">
                <h3 class="text-lg font-semibold text-slate-800 dark:text-white">Form Edit Slider Gambar</h3>
            </div>
            ...
            <form @submit.prevent="submit" class="p-6 space-y-6">
                ...
                <div>
                    ...
                    <div v-if="previewImage || existingImage" class="mb-4">
                        <div class="relative inline-block">
                            <img 
                                :src="previewImage || `/storage/${existingImage}`" 
                                alt="Preview" 
                        ...
                        <p class="mt-2 text-xs text-slate-500 dark:text-slate-400">
                            {{ previewImage ? 'Gambar baru (belum disimpan)' : 'Gambar saat ini' }}
                        </p>
                    </div>

                    <!-- Upload Button -->
                    <div class="flex items-center justify-center w-full">
                        <label for="gambar" class="flex flex-col items-center justify-center w-full h-32 border-2 border-slate-300 dark:border-slate-600 border-dashed rounded-lg cursor-pointer bg-slate-50 dark:bg-slate-700/50 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors">
                            <div class="flex flex-col items-center justify-center pt-5 pb-6">
                                <svg class="w-8 h-8 mb-2 text-slate-400 dark:text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
                                </svg>
                                <p class="mb-1 text-sm text-slate-500 dark:text-slate-400">
                                    <span class="font-semibold">Klik untuk ganti gambar</span>
                                </p>
                ...
                <div class="flex items-center justify-end space-x-3 pt-6 border-t border-slate-200 dark:border-slate-700">
                    ...
                    <button 
                        ...
                        {{ form.processing ? 'Menyimpan...' : 'Update Slider' }}
                    </button>
                </div>
            </form>
        </div>
