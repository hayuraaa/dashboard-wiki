...
const deleteCategory = (id) => {
    if (confirm('Apakah Anda yakin ingin menghapus kategori slider ini?')) {
        router.delete(`/data-master/slider-categories/${id}`);
    }
};
</script>

<template>
    <Head title="Kategori Slider" />

    <AdminLayout>
        <template #title>Kategori Slider</template>
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h2 class="text-xl md:text-2xl font-bold text-slate-800 dark:text-white">
                        Daftar Kategori Slider
                    </h2>
                    <p class="text-sm text-slate-600 dark:text-slate-400 mt-1">
                        Kelola kategori slider untuk berbagai halaman situs
                    </p>
                </div>
                <Link v-if="can('create slider categories')" href="/data-master/slider-categories/create"
                ...
                <div class="flex items-start gap-3">
                    ...
                    <div class="flex-1">
                        ...
                        <p class="text-sm text-purple-800 dark:text-purple-300 leading-relaxed">
                            Kategori slider digunakan untuk mengelompokkan slider berdasarkan halaman atau lokasi tampilan. 
                            Contoh: Slider Landing Page, Slider Kontak, dll.
                        </p>
                    </div>
                    <button type="button"
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50">
                <h3 class="text-lg font-semibold text-slate-800 dark:text-white">Daftar Kategori Slider</h3>
            </div>
            ...
            <div class="overflow-x-auto">
                        ...
                        <tr v-for="category in categories" :key="category.id"
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <div class="flex items-center justify-center space-x-2">
                                    <Link v-if="can('edit slider categories')" :href="`/data-master/slider-categories/${category.id}/edit`"
                                    ...
                                    <button v-if="can('delete slider categories')" @click="deleteCategory(category.id)"
                ...
                <div v-if="categories.length === 0" class="text-center py-12">
                    ...
                    <h3 class="text-lg font-medium text-slate-900 dark:text-white mb-2">
                        Belum ada kategori slider
                    </h3>
                    <p class="text-slate-500 dark:text-slate-400 mb-6">
                        Mulai menambahkan kategori untuk mengelompokkan slider
                    </p>
                    <Link href="/data-master/slider-categories/create"
