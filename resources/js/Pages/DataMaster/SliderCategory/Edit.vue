...
const form = useForm({
    ...
    keterangan: props.category.keterangan,
    is_active: props.category.is_active
});

const submit = () => {
    form.put(`/data-master/slider-categories/${props.category.id}`, {
        preserveScroll: true,
    });
};
</script>
...
<template>
    <Head title="Edit Kategori Slider" />

    <AdminLayout>
        <template #title>Edit Kategori Slider</template>
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h2 class="text-xl md:text-2xl font-bold text-slate-800 dark:text-white">
                        Edit Kategori Slider
                    </h2>
                    <p class="text-sm text-slate-600 dark:text-slate-400 mt-1">
                        Perbarui informasi kategori slider
                    </p>
                </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50">
                <h3 class="text-lg font-semibold text-slate-800 dark:text-white">Form Edit Kategori Slider</h3>
            </div>
