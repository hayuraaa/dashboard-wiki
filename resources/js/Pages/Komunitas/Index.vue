...
<template>
    ...
    <AdminLayout>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
                    ...
                    <tbody class="bg-white dark:bg-slate-800 divide-y divide-slate-200 dark:divide-slate-700">
                        <tr v-for="item in komunitas" :key="item.id" class="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors duration-150">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    <div class="w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 overflow-hidden bg-slate-100 dark:bg-slate-700">
                                        <img v-if="item.logo" :src="`/storage/${item.logo}`" :alt="item.nama_komunitas" class="w-full h-full object-cover">
                                        <svg v-else class="w-6 h-6 text-slate-400 dark:text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
                                        </svg>
                                    </div>
                                    <div class="ml-4">
                                        <div class="text-sm font-medium text-slate-900 dark:text-white">{{ item.nama_komunitas }}</div>
                                        <div v-if="item.alamat" class="text-sm text-slate-500 dark:text-slate-400 line-clamp-1">{{ item.alamat }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span :class="getBadgeColor(item.jenis_komunitas)" class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium">
                                    {{ item.jenis_komunitas }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-slate-900 dark:text-slate-200">{{ item.surel || '-' }}</div>
                                <div v-if="item.telepon" class="text-sm text-slate-500 dark:text-slate-400">{{ item.telepon }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
        ...
        <!-- Map View -->
        <div v-if="viewMode === 'map'" class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
            <div class="relative">
                <div ref="mapContainer" class="w-full h-[700px]"></div>

                <!-- List Komunitas Proyek/KlubWiki - Bottom Right Inside Map -->
                <div v-if="komunitasWithoutCoordinates.length > 0" class="absolute bottom-4 right-4 bg-white dark:bg-slate-800 rounded-lg shadow-lg border border-slate-200 dark:border-slate-700 p-4 z-[1000] w-80 max-h-96">
                    <h3 class="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-3 flex items-center border-b border-slate-200 dark:border-slate-700 pb-2">
                    ...
                    <div class="space-y-2 max-h-72 overflow-y-auto pr-1">
                        <Link v-for="item in komunitasWithoutCoordinates" :key="item.id" :href="`/list-komunitas/${item.id}`"
                            ...
                            <div class="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 overflow-hidden bg-slate-100 dark:bg-slate-700 mr-2">
                                <img v-if="item.logo" :src="`/storage/${item.logo}`" :alt="item.nama_komunitas" class="w-full h-full object-cover">
                                <svg v-else class="w-4 h-4 text-slate-400 dark:text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
                                </svg>
                            </div>
                            <div class="flex-1 min-w-0">
                                <div class="flex items-start justify-between gap-2">
                                    <h4 class="text-xs font-medium text-slate-900 dark:text-white line-clamp-1 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{{ item.nama_komunitas }}</h4>
                                    <span :class="getBadgeColor(item.jenis_komunitas)" class="inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium flex-shrink-0">
                                        {{ item.jenis_komunitas }}
                                    </span>
                                </div>
                                <p v-if="item.deskripsi" class="text-xs text-slate-500 dark:text-slate-400 mt-0.5 line-clamp-1">{{ item.deskripsi }}</p>
                            </div>
                            <svg class="w-3 h-3 text-slate-400 ml-1.5 flex-shrink-0 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor">
