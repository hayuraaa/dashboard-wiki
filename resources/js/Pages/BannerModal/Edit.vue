...
const form = useForm({
    judul: props.banner.judul,
    deskripsi: props.banner.deskripsi,
    gambar: null,
    url_pranala: props.banner.url_pranala,
    text_tombol: props.banner.text_tombol,
    tipe: props.banner.tipe,
    target_situs: props.banner.target_situs || [],
    is_active: props.banner.is_active,
    urutan: props.banner.urutan,
});

const gambarPreview = ref(null);
const existingGambar = ref(props.banner.gambar);

onMounted(() => {
    if (existingGambar.value) {
        gambarPreview.value = `/storage/${existingGambar.value}`;
    }
});
...
const submit = () => {
    form.put(`/banner-modals/${props.banner.id}`, { // Changed from form.post to form.put
        preserveScroll: true,
    });
};
</script>
...
<template>
    <Head title="Edit Banner Modal" />

    <AdminLayout>
        <template #title>Edit Banner Modal</template>
        ...
        <div class="mb-6 md:mb-8">
            <nav class="flex items-center space-x-2 text-sm">
                ...
                <span class="text-slate-900 dark:text-white font-medium">Edit Banner</span>
            </nav>
        </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
            ...
            <div class="px-6 py-4 border-b border-slate-200 dark:border-slate-700 bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-900/20 dark:to-purple-900/20">
                <div class="flex items-center">
                    ...
                    <div>
                        <h3 class="text-lg font-semibold text-slate-800 dark:text-white">Edit Informasi Banner</h3>
                        <p class="text-sm text-slate-600 dark:text-slate-400">Perbarui data banner modal</p>
                    </div>
                </div>
            </div>
                    ...
                    <div class="grid grid-cols-1 gap-6">
                        ...
                        <div>
                            ...
                            <label class="cursor-pointer inline-flex items-center px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-lg text-sm font-medium text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-700 hover:bg-slate-50 dark:hover:bg-slate-600 transition-colors">
                                ...
                                {{ existingGambar ? 'Ganti Gambar' : 'Pilih Gambar' }}
